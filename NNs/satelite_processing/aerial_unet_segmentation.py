...
def load_masks_and_patchify(directory_path, patch_size):
    ...
    for file_number, filepath in tqdm(enumerate(os.listdir(directory_path))):
        ...
        if extension == "jpg" or extension == "png":
            ...
            image[np.all(image==Background, axis = -1)] = (255, 255, 255)
            image[np.all(image==Building, axis = -1)] = (255, 0, 0)
            image[np.all(image==Road, axis = -1)] = (255, 255, 0)
            image[np.all(image==Water, axis = -1)] = (0, 0, 255)
            image[np.all(image==Barren, axis = -1)] = (159, 129, 183)
            image[np.all(image==Forest, axis = -1)] = (0, 255, 0)
            image[np.all(image==Agricultural, axis = -1)] = (255, 195, 128)
    ...
    for path, directories, files in os.walk(root_directory):
        for subdirectory in directories:
            ...
            # extract training label masks and patchify
                ...
                mask_dataset.extend(
                    load_masks_and_patchify(os.path.join(path, subdirectory), patch_size=patch_size))
